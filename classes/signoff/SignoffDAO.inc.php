...
class SignoffDAO extends DAO {
	...
	function signoffExists($symbolic, $assocType, $assocId, $userId = null, $stageId = null, $userGroupId = null) {
		...
		$params = array($symbolic, $assocType, $assocId);
	...
	function _fromRow(&$row) {
		...
		$signoff->setStageId($row['publication_stage_id']);
	...
	function insertObject(&$signoff) {
		$this->update(
			sprintf(
				...
				(symbolic, assoc_type, assoc_id, user_id, file_id, file_revision, date_notified, date_underway, date_completed, date_acknowledged, publication_stage_id, user_group_id)
				VALUES
				(?, ?, ?, ?, ?, ?, %s, %s, %s, %s, ?, ?)',
	...
	function &getUsersBySymbolic($symbolic, $assocType, $assocId, $stageId = null, $userGroupId = null) {
		...
		if ($stageId) {
			$sql .= ' AND s.publication_stage_id = ?';
			$params[] = $stageId;
		}
